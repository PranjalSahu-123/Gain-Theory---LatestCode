...
Module ProgramTVR
    ...
    Public Function ConstructInputXMLForProgTVR()
        ...
        Try
         ...
         <day_parts></day_parts>
            ...
            For index = 0 To Globals.Ribbons.MSprintExRibbon.tpSelections.UcMarkets1.lbPlan.Items.Count - 1
                ...
                Dim calclogic As XElement = <calc logic="ProgTVR" display=<%= disp %>/>
            ...
            '    input.Add(TG_MGElement)
            'Next
            Globals.Ribbons.MSprintExRibbon.UpdateUsageReport("ProgramTVR WS", Globals.Ribbons.MSprintExRibbon.tpSelections.tbClientName.Text.Trim, Globals.Ribbons.MSprintExRibbon.tpSelections.tbBrandValue.Text.Trim, input, 0)
    ...
    End Function
    Public Function DisplayProgTVRDetailsOnSheet(ByVal ds As DataSet, Optional ByVal fromPane As Boolean = False)
        Try
            Dim plantgname As String = String.Empty
            Dim reftgname As String = String.Empty
            Dim dtable As System.Data.DataTable = DirectCast(Globals.Ribbons.MSprintExRibbon.tpSelections.UcAudience1.DgPlanRefGrid.DataSource, System.Data.DataTable)
            plantgname = dtable.Rows(0)(1).ToString().Trim()
            '  reftgname = dtable.Rows(1)(1).ToString()
            If Not CheckSheetExists("Program TVR") Then
            ...
            Else
                ...
                If fromPane Then
                    newSheet = ReturnActualSheet("Program TVR")
                    ' newSheet.UsedRange.Clear()
                    Globals.Ribbons.MSprintExRibbon.CleanSheet(newSheet)
                    newSheet.Activate()
                Else
                    Dim sheetcount As Integer = CheckAndReturnSheet("Program TVR")
                    If sheetcount > 0 Then
                        newSheet = DirectCast(Globals.ThisAddIn.Application.ActiveWorkbook.Worksheets.Add(, Globals.ThisAddIn.Application.ActiveSheet), Microsoft.Office.Interop.Excel.Worksheet)
                        Dim name As String = String.Format("Program TVR({0})", sheetcount)
                        newSheet.Name = name
                    End If
                End If
            ...
            For index = 0 To Globals.Ribbons.MSprintExRibbon.channels.Rows.Count - 1
                ...
                For index1 = 0 To ds.Tables.Count - 1
                    ...
                    '  Dim marketcell As Microsoft.Office.Interop.Excel.Range = vstoWorkbook.get_Range(vstoWorkbook.Cells(count + 2 + Convert.ToInt32(Globals.Ribbons.MSprintExRibbon.tpSelections.UcChannels.nudTopPrograms.Value + 4) * index, 7 * index1 + 1), vstoWorkbook.Cells(count + 2 + Convert.ToInt32(Globals.Ribbons.MSprintExRibbon.tpSelections.UcChannels.nudTopPrograms.Value + 4) * index, (7 * index1 + 1) * index1 + 1))
                    ...
                    'Globals.Ribbons.MSprintExRibbon.channels.Columns.Add("CName")
                    'Globals.Ribbons.MSprintExRibbon.channels.Columns.Add("StartDate")
                    '  Globals.Ribbons.MSprintExRibbon.channels.Columns.Add("EndDate")
                    ...
                    If dtrows.Count > 0 Then
                        ...
                        dt.Columns.RemoveAt(0)
                        dt.AcceptChanges()
                        listobject.DataSource = dt.AsEnumerable().Take(Globals.Ribbons.MSprintExRibbon.tpSelections.UcChannels.nudTopPrograms.Value).CopyToDataTable()
                    End If
                ...
                'If channel.Equals(Globals.Ribbons.MSprintExRibbon.channels.Rows(index)("CName").ToString()) Or index = 0 Then
                ...
                'channel = Globals.Ribbons.MSprintExRibbon.channels.Rows(index)("CName").ToString()
                count += 1
            Next
        Catch ex As Exception
            LogMpsrintExException("Exception occured while displaying Program TVR details." + ex.Message)
            Throw ex
        End Try
    End Function
End Module
