...
Public Class ucMkets
    ...
    Friend dtMarketsCopy As Data.DataTable
    ...
    Private Sub ucMkets_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            dtMarketsCopy = New Data.DataTable
            dtMarketsCopy = Globals.Ribbons.MSprintExRibbon.dtMarkets1.Copy
    ...
    'Private Sub btnSetRefMG_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
    '    If TabControl1.SelectedTab Is tpMarkets Then
    '        For Each item As System.Windows.Forms.DataGridViewRow In dgvMarkets.SelectedRows
    ...
    Private Sub lbRef_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
    ...
    'Private Sub lbRef_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
    '    If e.KeyCode = Windows.Forms.Keys.Delete Then
    '        Try
    ...
    Private Sub TextBox1_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TextBox1.TextChanged
        ...
        Try
            If Not (String.IsNullOrEmpty(TextBox1.Text.Trim())) Then
                ...
                dgvMarkets.DataSource = Nothing
                Dim dt As Data.DataTable = New Data.DataTable
                dt = dtMarketsCopy.Clone()
                '   dt.Columns.Add("Name")
                For Each row As Data.DataRow In dtMarketsCopy.Rows

                    If row("Name").ToString().ToUpper().StartsWith(TextBox1.Text.Trim().ToUpper()) Then
                        '  clbSelectChannels.Items.Add(row(0).ToString())
                        Dim newrow As Data.DataRow = dt.NewRow
                        newrow(1) = row("Name").ToString()
                        dt.Rows.Add(newrow)
                    End If

                Next
                dgvMarkets.DataSource = dt
                dgvMarkets.Columns(0).Visible = False
                dgvMarkets.Columns(2).Visible = False
            ElseIf (TextBox1.Text.Trim().Length = 0) Then
                ...
                '   dt.Columns.Add("Name")
                dt = dtMarketsCopy.Clone()
                For Each dr As Data.DataRow In dtMarketsCopy.Rows
                    '  clbSelectChannels.Items.Add(dr(0).ToString())
                    '  clbSelectChannels.Items.Add(row(0).ToString())
                    Dim newrow As Data.DataRow = dt.NewRow
                    newrow(1) = dr("Name").ToString()
                    dt.Rows.Add(newrow)
                Next
                dgvMarkets.DataSource = dt
                dgvMarkets.Columns(0).Visible = False
        ...
        Catch ex As Exception
            LogMpsrintExException("Exception occured while filtering Markets grid.Message: " + ex.Message)
        End Try
    End Sub
End Class
